	  ...
	  if (length() >= 2) {
		  ...
		  while (node.next != null) {
			  if (node.item.equals(node.next.item)) {
				  node.next = node.next.next; //remove the consecutive items, and keep the node at first item
				  size--;
  ...
  public void twin() {
    // Fill in your solution here.  (Ours is seven lines long.)
	  if (length() >= 1) {
		  SListNode node = head;
		  while (true) {
			  node.next = new SListNode(node.item, node.next); //insert the duplicate node
			  size++;
			  if (node.next.next == null) { //check the node after the duplicate node
				  return;
			  }
			  node = node.next.next;
		  }
